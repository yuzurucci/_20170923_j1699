...
STATUS VerifyGroupControlSupport(void)
{
	...
	if (SidRequest(&SidReq, SID_REQ_NORMAL) == FAIL)
	{
		Log( FAILURE, SCREENOUTPUTON, LOGOUTPUTON, NO_PROMPT,
		     "SID $8 group ($00-$A0) support request failed\n");
		bTestFailed = TRUE;
	}
	else
	{
		...
		for (EcuIndex = 0; EcuIndex < gOBDNumEcus; EcuIndex++)
		{
			...
			{
				Log( FAILURE, SCREENOUTPUTON, LOGOUTPUTON, NO_PROMPT,
					 "ECU %X  SID $8 group/individual support response size mismatch\n", GetEcuId(EcuIndex) );
				bTestFailed = TRUE;
			}
			...
			for (IdIndex = 0; IdIndex < 8; IdIndex++)
			{
				for (IdBitIndex = 0; IdBitIndex < 4; IdBitIndex++)
				{
					...
					{
						Log( FAILURE, SCREENOUTPUTON, LOGOUTPUTON, NO_PROMPT,
							 "ECU %X  SID $8 TID $%02X Byte %d group/individual response mismatch\n",
							 GetEcuId(EcuIndex),
							 gOBDCompareResponse[EcuIndex].Sid8TidSupport[IdIndex].FirstID,
							 IdBitIndex );
						bTestFailed = TRUE;
					}
				}
