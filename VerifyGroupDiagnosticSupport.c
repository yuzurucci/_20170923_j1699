...
STATUS VerifyGroupDiagnosticSupport(void)
{
	...
	if (SidRequest(&SidReq, SID_REQ_NORMAL) == FAIL)
	{
		Log( FAILURE, SCREENOUTPUTON, LOGOUTPUTON, NO_PROMPT,
		     "SID $1 group ($00-$A0) support request failed\n");
		bTestFailed = TRUE;
	}
	else
	{
		...
		if ( bReqNextGroup == 1 )
		{
			...
			{
				Log( FAILURE, SCREENOUTPUTON, LOGOUTPUTON, NO_PROMPT,
					 "SID $1 group ($C0-$E0) support request failed\n");
				bTestFailed = TRUE;
			}
		}
		...
		for (EcuIndex = 0; EcuIndex < gOBDNumEcus; EcuIndex++)
		{
			for (IdIndex = 0; IdIndex < 8; IdIndex++)
			{
				for (IdBitIndex = 0; IdBitIndex < 4; IdBitIndex++)
				{
					...
					{
						Log( FAILURE, SCREENOUTPUTON, LOGOUTPUTON, NO_PROMPT,
							 "ECU %X  SID $1 PID $%02X Byte %d group/individual support response mismatch\n",
							 GetEcuId(EcuIndex),
			...
			{
				Log( FAILURE, SCREENOUTPUTON, LOGOUTPUTON, NO_PROMPT,
					 "ECU %X  SID $1 PID Supported PID group/individual size mismatch\n",
					 GetEcuId(EcuIndex) );
				bTestFailed = TRUE;
			}
		}
